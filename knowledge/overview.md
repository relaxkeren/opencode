...
| `knowledge/model-architecture.md` | Model architecture | How Connect provider, Switch model, and server-side provider/auth work; how the app connects to each provider (no LiteLLM gateway). (source: knowledge/model-architecture.md) | - Connect provider flow (TUI + web) (source: knowledge/model-architecture.md#2-connect-provider-flow)<br>- Server-side provider state and config.providers (source: knowledge/model-architecture.md#3-server-side-provider-and-auth)<br>- Vercel AI SDK providers, OpenAI-compatible, dynamic loading (source: knowledge/model-architecture.md#4-how-the-app-connects-to-each-provider)<br>- How models become selectable in TUI and web (source: knowledge/model-architecture.md#5-how-models-become-selectable) | References packages/opencode server, auth, provider, TUI dialog-model. (source: knowledge/model-architecture.md) | High |
| `knowledge/ui-architecture.md` | TUI architecture | How the CLI terminal UI is built: SolidJS + OpenTUI, why TSX looks like markup, rendering pipeline, directory layout. (source: knowledge/ui-architecture.md) | - SolidJS + OpenTUI stack; render() and providers (source: knowledge/ui-architecture.md#3-rendering-pipeline)<br>- OpenTUI APIs (@opentui/solid, @opentui/core) (source: knowledge/ui-architecture.md#4-key-opentui-apis-used-in-the-codebase)<br>- TUI directory layout (app, context, routes, component, ui, util) (source: knowledge/ui-architecture.md#5-directory-layout-tui)<br>- SDK/Sync and server communication (source: knowledge/ui-architecture.md#6-communication-with-the-server) | References packages/opencode/src/cli/cmd/tui; links knowledge/model-architecture.md. (source: knowledge/ui-architecture.md) | High |
